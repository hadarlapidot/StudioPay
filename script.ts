const tattooPriceInput = document.getElementById("tattoo-price") as HTMLInputElement;
const sessionPriceInput = document.getElementById("session-price") as HTMLInputElement;
...
function checkValidity(tattoo: number, session: number): boolean{
  const errorElement = document.getElementById("error-msg") as HTMLDivElement | null;

// check nulity
if(errorElement)

  if(tattoo < session)  {
    if(errorElement) errorElement.textContent = "Session price must be less or uqual to the tattoo price"
    return false;
...
function calculateResults() {
  ...
  if(!checkValidity(tattooPrice, sessionPrice)) throw new Error("Input is invalid");

  // Calculate studio fees
  const firstSessionFee:number = (0.3 * sessionPrice < 650) ? parseFloat((0.3 * sessionPrice).toFixed(2)) : 650;
  const secondSessionFee: number = 0.3 * (tattooPrice - sessionPrice) < 650 ? parseFloat((0.3 * (tattooPrice - sessionPrice)).toFixed(2)) : 650;
  ...
  } else throw new Error("TotalAmount, firstSessionFee or secondSessionFee HTML elements are unavailabe");
 ...
 { 
    ...
    throw new Error("Element 'net worth' could not be found..." );
...
{  const minFee = Math.min(650, 0.3 * tattooPrice);
  ...
  else throw new Error("could not find element 'netWorth' for some reason...");
}
